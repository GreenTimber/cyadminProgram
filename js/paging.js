$(function () {
  var $oResultBox = $('#jitairesultBox');
  // var books = data.books;
  var DATACOUNT =200;
  $('#paging').paging({
    nowPage: 1,
    allPages: Math.ceil(DATACOUNT / 36),
    displayPage: 10,
    callBack: function (now) {
      var currentPages = now * 36 < DATACOUNT ? 36 : DATACOUNT - (now - 1) * 36;
      ...
      for (var i = 1; i <= currentPages; i++) {
        ...
        var create_dl = $('<div class="col-xs-3 col-sm-3 col-md-2 col-lg-1 colmarign" ></div>');
        var _html = ' <input type="image" class="jtico"  src="images/machine.ico" onclick="showjttable('+num+')" />'+
          ' <div class="state'+state+'"></div>'+
          '<label class="jitainumber">编号：'+num+'</label>';

        create_dl.html(_html).addClass('zoomIn animated');
